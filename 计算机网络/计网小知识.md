...
1. **缓存处理**
   ...
   **If-Modified-Since** 则是由客户端发送给服务器的HTTP请求头标签
   ...
   ![image-20210908155940688](https://cdn.jsdelivr.net/gh/dopamine-joker/image-host/image/image-20210908155940688.png)
...
![image-20210908155346372](https://cdn.jsdelivr.net/gh/dopamine-joker/image-host/image/image-20210908155346372.png)

仔细观察上图：在tcp链接中，http请求必须等待前一个请求响应之后，才能发送，后面的依次类推，由此可以看出，如果在一个tcp通道中如果某个http请求的响应因为某个原因没有及时返回，后面的响应会被阻塞，这就是**队头阻塞**。

![image-20210908155439291](https://cdn.jsdelivr.net/gh/dopamine-joker/image-host/image/image-20210908155439291.png)

观察上图红线标识的响应，因为红线标识的响应被阻塞了，它后面的所有响应都会被阻塞，这就是**队头阻塞**。

![image-20210908155726095](https://cdn.jsdelivr.net/gh/dopamine-joker/image-host/image/image-20210908155726095.png)

![image-20210908155716352](https://cdn.jsdelivr.net/gh/dopamine-joker/image-host/image/image-20210908155716352.png)
...
3. **多路复用**
    ...
    在HTTP1.0中，我们经常会使用到雪碧图、使用多个域名等方式来进行优化，都是因为浏览器限制了同一个域名下的请求数量，当页面需要请求很多资源的时候，HTTP队头阻塞（Head of line blocking）会导致在达到最大请求时，资源需要等待其他资源请求完成后才能继续发送。
...
4. **服务器push**
   ...
   ![image-20211031224916732](https://cdn.jsdelivr.net/gh/dopamine-joker/image-host/202110312249819.png)

   - 服务端推送的过程：

       ![image-20211031224946770](https://cdn.jsdelivr.net/gh/dopamine-joker/image-host/202110312249844.png)
...
3. **<font color=red>QUIC怎么解决TCP队头阻塞</font>**
    ...
    **下图b中由于本质还是使用TCP传输，由于滑动窗口问题，从而导致其后面的TCP数据包也被阻塞，而这部分阻塞的TCP数据就包括了各个stream中的信息。**
    ...
    ![image-20211031230850898](https://cdn.jsdelivr.net/gh/dopamine-joker/image-host/202110312308018.png)
    ...
    **有了<font color=red>Stream Offset</font> 字段信息，属于同一个Stream ID 的数据包也可以乱序传输了**（**HTTP/2 中仅靠Stream ID 标识，要求同属于一个Stream ID 的数据帧必须有序传输**），通过两个数据包的Stream ID 与 Stream Offset 都一致，就说明这两个数据包的内容一致。
    ...
    ![image-20211031231442613](https://cdn.jsdelivr.net/gh/dopamine-joker/image-host/202110312314707.png)
...
4. **<font color=red>QUIC如何减少TCP连接的开销</font>**
    ...
    ![img](https://cdn.jsdelivr.net/gh/dopamine-joker/image-host/202111010055491.gif)
...
1. **域名解析过程**
   ...
   **顶级域(Top-Level domain, TLD)服务器**: 诸如(com,org,net,edu和gov)和所有国家顶级域(uk,fr,ca,jp)
   ...
   ![image-20210911103335946](https://cdn.jsdelivr.net/gh/dopamine-joker/image-host/image/image-20210911103335946.png)
   
   ![image-20210911103322935](https://cdn.jsdelivr.net/gh/dopamine-joker/image-host/image/image-20210911103322935.png)
...
![qm](https://cdn.jsdelivr.net/gh/dopamine-joker/image-host/image/qm.png)
...
![image-20210908155840188](https://cdn.jsdelivr.net/gh/dopamine-joker/image-host/image/image-20210908155840188.png)
...
![image-20210908164734355](https://cdn.jsdelivr.net/gh/dopamine-joker/image-host/image/image-20210908164734355.png)
...
![image-20210908155847097](https://cdn.jsdelivr.net/gh/dopamine-joker/image-host/image/image-20210908155847097.png)

# 35. ICMP

原文链接: https://blog.csdn.net/m0_37268841/article/details/109165302

![image-20210908160944359](https://cdn.jsdelivr.net/gh/dopamine-joker/image-host/image/image-20210908160944359.png)
...
共同实现DNS分布式数据库的所有DNS服务器存储了**资源记录(RR)**,RR**提供了主机名到IP地址的映射**
...
- 时间等待错误
    ...
    ![1479723525](https://cdn.jsdelivr.net/gh/dopamine-joker/image-host/202110250836484.jpg)
